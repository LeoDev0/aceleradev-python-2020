...
def classify_by_phone_number(records):
    ...
    for record in records:
        initial_hour = int(datetime.fromtimestamp(record['start']).strftime('%H'))
        total_minutes = (record['end'] - record['start']) // 60
        total_bill = PERMANENT_FEE
        if 6 <= initial_hour <= 22:
            total_bill += (total_minutes * 0.09)
        ...
        if not repeated_source:
            final_bill.append({'source': record['source'], 'total': round(total_bill, 2)})
